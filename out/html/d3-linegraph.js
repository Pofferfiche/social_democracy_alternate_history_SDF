...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    /* params */
    if (!parties) {
        parties = ['sdf', 'nlu', 'npfp', 'la', 'cua', 'bnu', 'other'];
    }
    if (!partyColors) {
        partyColors = {'sdf': '#E3000F', 'nlu': '#E8D023', 'npfp': '#45B332', 'la': '#C9C187', 'cua': '#4E90BF', 'bnu': '#0F1A4A', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'sdf': 'SDF', 'nlu': 'NLU', 'npfp': 'npfp', 'la': 'LA', 'cua': 'CUA', 'bnu': 'BNU', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          const maxSDF = d3.max(data, d => d.sdf);
          const maxBNU = d3.max(data, d => d.bnu);
          dataMax = maxSDF >= maxBNU ? maxSDF + 10 : maxBNU + 10;
          dataMin = 0;
      }
